...
class WaitingKeywords(LibraryComponent):
    ...
    @keyword
    def wait_until_element_attribute_contains(self, locator, attr, contains_val, timeout=None,
                                    error=None):
        """Waits until element ``locator`` attribute contains ``text``.
        ...
        if attr.lower() == 'text':
            self._wait_until(
                lambda: contains_val in self.find_element(locator).text,
                "Element '%s' did not get text '%s' in <TIMEOUT>." % (locator, contains_val),
                timeout, error
            )
        else:
            self._wait_until(
                lambda: contains_val in self.find_element(locator).get_attribute(attr.lower()),
                "Element '%s' did not get Attribute %s value %s' in <TIMEOUT>." % (locator, attr, contains_val),
                timeout, error
            )
